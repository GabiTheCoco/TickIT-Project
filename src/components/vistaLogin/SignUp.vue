  ...
  <div class="flex items-center justify-center bg-white-900 pt-0 w-full h-[90%]">
    <div class="flex flex-col justify-between py-10 p-8 2xl:py-16 rounded-lg bg-azul-oscuro  min-w-[24rem] 2xl:max-w-[28rem] min-h-[28rem] 2xl:max-h-[32rem] 2xl:h-5/6">
      ...
      <form class="flex flex-col justify-center gap-5">
          <input v-model="nombre" class="p-2  rounded-lg w-full outline-white/60" placeholder="Nombre" type="text">
          <input v-model="email" class="p-2  rounded-lg w-full outline-white/60" placeholder="Email" type="email">
          <input v-model="password" class="p-2 rounded-lg w-full  outline-white" placeholder="Contraseña" type="password">
      </form>
      <div class="flex justify-between gap-5">
          <button @click="register" class="bg-white hover:bg-gray-200 text-gray-800 font-semibold p-2 rounded-lg w-full">Registrarse</button>
          <button @click="SignInWithGoogle" type="button"  class="bg-white hover:bg-gray-200 text-gray-800 font-semibold rounded-lg px-1">
