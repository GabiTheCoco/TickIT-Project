    ...
    <div class="flex flex-col justify-between py-10 p-8 2xl:py-16 rounded-lg bg-azul-oscuro  min-w-[24rem] 2xl:max-w-[30rem] min-h-[28rem] 2xl:max-h-[32rem] 2xl:h-5/6">
      ...
      <form class="flex flex-col justify-center gap-10 h-5/6">
        <div class="flex flex-col justify-center gap-4">
          ...
          <input v-model="password" class="p-2 rounded-lg w-full outline-white/60" placeholder="Contraseña" type="password">
          <p class="text-sm text-center w-full text-white">{{ errorMsg.value }}</p>
        </div>
        <div class="flex justify-between gap-5">
          <button @click.prevent="SignIn" class="bg-white hover:bg-gray-200 text-gray-800 font-semibold p-2 rounded-lg w-full">Iniciar sesión</button>
          <button type="button" @click="SignInWithGoogle" class="bg-white hover:bg-gray-200 text-gray-800 font-semibold rounded-lg px-1">
